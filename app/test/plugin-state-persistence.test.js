...
const userConfigsDir = path.join(baseDir, 'test_user_configs');
const profileName = 'testprofile';
const stateFile = path.join(userConfigsDir, `${profileName}_plugins_state.json`);
...
// Ensure user config directory exists for profile-scoped state
...
const mockApp = {
    ...
    put: () => {},
    delete: () => {},
    // PluginLoader mounts a router, so we need a no-op use()
    use: () => {}
};
...
async function runTest() {
    try {
        ...
        console.log('--------------------------------------');
        const loader1 = new PluginLoader(pluginsDir, mockApp, mockIO, mockDB, mockLogger, mockConfigPathManager, profileName);
        assert.strictEqual(loader1.stateFile, stateFile, 'State file should be profile-scoped');
        ...
        const loader2 = new PluginLoader(pluginsDir, mockApp, mockIO, mockDB, mockLogger, mockConfigPathManager, profileName);
