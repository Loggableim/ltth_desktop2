...
The Gift HUD Rotator feature allows you to showcase TikTok gifts on your OBS overlay in a rotating carousel format.

### Features

- **Visual Gift Catalog**: Browse and select gifts from TikTok's gift catalog with image previews
- **Search & Filter**: Quickly find gifts by name or ID
- **10 Design Templates**: Choose from various card styles:
  - **Card**: Classic bordered card style (default)
  - **Banner**: Compact horizontal layout
  - **Minimal**: Clean design with no border
  - **Modern Glass**: Frosted glass effect with backdrop blur
  - **Neon Glow**: Vibrant glowing border effect
  - **Elegant**: Refined style with soft shadows
  - **Bold**: Strong uppercase text with thick borders
  - **Compact**: Space-efficient smaller layout
  - **Wide Banner**: Extended horizontal display
  - **Gradient**: Colorful gradient background

- **10 Transition Animations**: Smooth animations when gifts rotate:
  - **Fade**: Classic fade in/out
  - **Slide Left/Right/Up/Down**: Directional slide transitions
  - **Zoom**: Scale up animation
  - **Flip**: 3D flip effect
  - **Rotate**: Spinning entrance
  - **Bounce**: Playful bouncing entrance
  - **Swing**: Swinging from top

- **Full Customization**: 
  - Custom headlines and descriptions for each gift
  - Font selection (Inter, Montserrat, Oswald, Poppins, Roboto, Raleway)
  - Text, accent, and background color customization
  - Adjustable rotation interval (3-60 seconds)

### Setup

1. Navigate to GCCE plugin settings → **HUD Overlay** tab
2. Scroll to **Gift HUD Rotator** section
3. Click **Load Gift Catalog** to fetch available gifts
4. Search or browse the visual gift catalog
5. Click on a gift to select it
6. Configure:
   - **Headline**: Custom title (defaults to gift name)
   - **Info Text**: Optional description
   - **Template**: Choose design style
   - **Animation**: Select transition effect
   - **Font**: Select typography
   - **Colors**: Customize text, accent, and background
7. Click **Add to Rotator**
8. Repeat for multiple gifts
9. Enable rotator and set rotation interval
10. Click **Save Rotator**

### OBS Integration

Add the rotator overlay to OBS:
...
The rotator appears in the **bottom-left corner** by default and automatically cycles through your configured gifts.

### API Endpoints

- `GET /api/gcce/hud/rotator` - Get current rotator configuration
- `POST /api/gcce/hud/rotator` - Update rotator configuration

### Socket Events

- `gcce:hud:rotator:update` - Emitted when rotator config changes (triggers overlay refresh)

### Tips

- Use **Modern Glass** or **Gradient** templates for a contemporary look
- Combine **Neon Glow** template with **Zoom** animation for impact
- Use **Compact** template if you need more screen space
- Match colors to your stream branding
- Keep rotation intervals between 5-15 seconds for best visibility
- Add 3-8 gifts for optimal rotation without repetition fatigue
