...
class GlobalChatCommandEngine {
    ...
    async init() {
        ...
        try {
            ...
            // Register custom media commands from media library
            this.registerCustomMediaCommands();
    ...
    registerBuiltInCommands() {
        ...
        // /hudmedia command (gif/mp4)
        this.registry.registerCommand({
            pluginId: 'gcce',
            name: 'hudmedia',
            description: 'Display custom GIF/MP4 on HUD overlay',
            syntax: '/hudmedia [duration] <name|url>',
            permission: this.hudManager.config.permissions.media || this.hudManager.config.permissions.image,
            enabled: this.hudManager.config.chatCommands.allowMedia,
            minArgs: 1,
            category: 'HUD',
            handler: async (args, context) => await this.hudManager.handleMediaCommand(args, context)
        });
     ...
     * Register custom media commands from media library
     * 
     * Note: This method unregisters all existing custom media commands before 
     * registering new ones to ensure the command registry stays in sync with 
     * the media library. This is called infrequently (only on init and when 
     * media library is saved), so the performance impact is negligible.
     */
    registerCustomMediaCommands() {
        const mediaLibrary = this.hudManager.getMediaLibrary();
        
        // First, unregister any existing custom media commands to ensure sync
        this.unregisterCustomMediaCommands();
        
        if (!Array.isArray(mediaLibrary)) {
            return;
        }

        // Register a command for each media item that has a custom command
        mediaLibrary.forEach(mediaItem => {
            if (!mediaItem.command) {
                return; // Skip if no custom command specified
            }

            // Extract command name (remove prefix if present)
            let commandName = mediaItem.command.trim();
            if (commandName.startsWith('/') || commandName.startsWith('!')) {
                commandName = commandName.substring(1);
            }

            if (!commandName) {
                return; // Skip if empty after removing prefix
            }
            ...
            try {
                this.registry.registerCommand({
                    pluginId: 'gcce',
                    name: commandName,
                    description: `Display ${mediaItem.label} media on HUD overlay`,
                    syntax: `${mediaItem.command}`,
                    permission: this.hudManager.config.permissions.media || this.hudManager.config.permissions.image,
                    enabled: this.hudManager.config.chatCommands.allowMedia,
                    minArgs: 0,
                    maxArgs: 1, // Optional duration override
                    category: 'HUD',
                    metadata: {
                        isCustomMediaCommand: true,
                        mediaId: mediaItem.id
                    },
                    handler: async (args, context) => {
                        // Handle optional duration override as first argument
                        const duration = args.length > 0 && !isNaN(parseFloat(args[0])) 
                            ? parseFloat(args[0]) 
                            : mediaItem.duration;

                        // Create media element directly
                        const elementId = this.hudManager.createMediaElement(
                            mediaItem.url,
                            duration,
                            mediaItem.type,
                            context,
                        ...
                        return {
                            success: true,
                            message: `${mediaItem.label} displayed for ${duration} seconds`,
                            data: { elementId, duration }
                        };
                    }
                });

                this.api.log(`[GCCE] Registered custom media command: ${commandName}`, 'debug');
            } catch (error) {
                this.api.log(`[GCCE] Failed to register custom media command ${commandName}: ${error.message}`, 'error');
            }
        });
    }

    /**
     * Unregister all custom media commands
     */
    unregisterCustomMediaCommands() {
        const allCommands = this.registry.getAllCommands();
        const customMediaCommands = allCommands.filter(cmd => 
            cmd.metadata && cmd.metadata.isCustomMediaCommand
        );

        customMediaCommands.forEach(cmd => {
            try {
                this.registry.unregisterCommand(cmd.name, 'gcce');
                this.api.log(`[GCCE] Unregistered custom media command: ${cmd.name}`, 'debug');
            } catch (error) {
                this.api.log(`[GCCE] Failed to unregister custom media command ${cmd.name}: ${error.message}`, 'error');
            }
        });
    }
    ...
    registerRoutes() {
        ...
        // Test media display
        this.api.registerRoute('POST', '/api/gcce/hud/test/media', async (req, res) => {
            try {
                const { url, duration } = req.body;
                const context = { userId: 'test-user', username: 'Test User' };
                const result = await this.hudManager.handleMediaCommand(
                    [duration || '8', url || ''],
                    context
                );
                res.json(result);
        ...
        // Media library
        this.api.registerRoute('GET', '/api/gcce/hud/media', async (req, res) => {
            try {
                res.json({ success: true, media: this.hudManager.getMediaLibrary() });
            } catch (error) {
                res.status(500).json({ success: false, error: error.message });
            }
        });

        this.api.registerRoute('POST', '/api/gcce/hud/media', async (req, res) => {
            try {
                const media = await this.hudManager.updateMediaLibrary(req.body.media || []);
                
                // Re-register custom media commands
                this.registerCustomMediaCommands();
                
                // Emit event to notify clients
                this.api.emit('gcce:media_library_updated', { timestamp: Date.now() });
        ...
        // Gift rotator config
        this.api.registerRoute('GET', '/api/gcce/hud/rotator', async (req, res) => {
            try {
                res.json({ success: true, rotator: this.hudManager.getGiftRotator() });
            } catch (error) {
                res.status(500).json({ success: false, error: error.message });
            }
        });

        this.api.registerRoute('POST', '/api/gcce/hud/rotator', async (req, res) => {
            try {
                const rotator = await this.hudManager.updateGiftRotator(req.body.rotator || req.body || {});
                const emitter = this.socketBatcher || this.api.getSocketIO();
                if (emitter && emitter.emit) {
                    emitter.emit('gcce:hud:rotator:update', rotator);
                }
    ...
    async handleChatMessage(data) {
        try {
            ...
            const context = {
                ...
                // Add fields for game-engine compatibility
