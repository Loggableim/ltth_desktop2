...
- **TikTok Events Integration**:
  - Gifts - Add/remove time based on coin value (configurable seconds per coin)
  - Likes - Modify timer based on likes received (configurable seconds per like)
...
- **Configurable Ratios** - Set seconds-per-coin for gifts and seconds-per-like for likes
...
4. Examples:
   - "Gift event: 0.1 seconds per coin" (a 100-coin gift adds 10 seconds)
   - "Like event: 0.1 seconds per like" (100 likes add 10 seconds)
...
Event Type: Gift
Action: Add Time
Value: 0.1 seconds per coin
Conditions:
  - Gift Name: Rose (optional - specific gift)
  - Min Coins: 100 (optional - minimum value)
```

Example: If you set 0.1 seconds per coin and someone sends a 100-coin gift, 10 seconds will be added (0.1 × 100 = 10).

**Fractional Seconds:** You can use fractional values like 0.2, 0.5, or 1.5 seconds per coin. When a gift is sent multiple times (e.g., "Rose x10"), the system calculates: `seconds_per_coin × coins × repeat_count`. For example:
- 0.2 seconds per coin
- Rose = 1 coin
- Sent 10 times (x10)
- Result: 0.2 × 1 × 10 = 2.0 seconds added
...
Example: If you set 0.1 seconds per like and someone sends 50 likes, 5 seconds will be added (0.1 × 50 = 5).
...
// Apply profile
POST /api/advanced-timer/profiles/:id/apply
```

**Note:** All time values support fractional seconds (e.g., 0.2 seconds per gift).
...
- **Recovery** - Timers restored to last known state on restart
  - Running timers automatically resume
  - Paused timers remain paused (ready to resume)
  - Stopped/completed timers remain in their state
  - All target and initial duration values preserved
- **Fractional Precision** - Database stores fractional seconds (0.1s, 0.5s, etc.) without rounding loss
...
### Flows/Events System
Advanced Timer registers comprehensive flow actions:

**Timer Control:**
- Start Timer - Begin countdown/count up
- Pause Timer - Pause a running timer
- Resume Timer - Continue a paused timer (NEW)
- Stop Timer - Stop and reset timer
- Reset Timer - Reset to initial value

**Timer Modification:**
- Add Time to Timer - Add seconds (supports fractional)
- Remove Time from Timer - Remove seconds (supports fractional)
- Set Timer Value - Jump to specific value in seconds (NEW)

**Timer Management:**
- Create Timer - Create new timer with name, mode, duration, target (NEW)
- Delete Timer - Delete timer by ID (NEW)
- Set Like Speed Ratio - Configure like-based speed modification (NEW)

**Profile Management:**
- Apply Timer Profile - Load and apply a saved profile (NEW)

**Automation:**
- Trigger Timer Chain - Manually execute a chain (NEW)
- Enable/Disable Timer Chain - Control chain activation (NEW)
- Enable/Disable Timer Rule - Control IF/THEN rule activation (NEW)

**Fractional Time Support:**
All time-based actions and gift events support fractional seconds (e.g., 0.2s per gift, 10x = 2.0s total).
