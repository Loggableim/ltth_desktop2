        ...
        try {
            ...
            for (const timerConfig of timers) {
                ...
                for (const event of events) {
                    ...
                    // Apply action
                    // action_value represents seconds per coin for gifts (can be fractional)
                    // repeatCount represents how many times the gift was sent
                    const actionValue = parseFloat(event.action_value) || 0;
                    const totalCoins = (coins || 0) * (repeatCount || 1);
                    const actualValue = actionValue * totalCoins;
                    ...
                    if (event.action_type === 'add_time') {
                        ...
                        this.plugin.db.addTimerLog(
                            ...
                            uniqueId,
                            actualValue,
                            `Gift: ${giftName} (${coins} coins) x${repeatCount || 1} = ${actualValue.toFixed(2)}s`
                        );
        ...
        try {
            ...
            for (const timerConfig of timers) {
                ...
                for (const event of events) {
                    ...
                    // Apply action
                    // action_value represents seconds per like
                    ...
                    if (event.action_type === 'add_time') {
                        ...
                        this.plugin.db.addTimerLog(
                            ...
                            uniqueId,
                            actualValue,
                            `Likes: ${likeCount} = ${actualValue.toFixed(2)}s`
                        );
                    } else if (event.action_type === 'remove_time') {
