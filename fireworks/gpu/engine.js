...
const CONFIG = {
    ...
    // Per-tier particle count caps to prevent performance degradation
    particleCaps: {
        small: 50,      // Small tier: max 50 particles
        medium: 100,    // Medium tier: max 100 particles
        big: 150,       // Big tier: max 150 particles
        massive: 200    // Massive tier: max 200 particles
    },
    // Off-screen culling margins (particles beyond this are removed)
    cullMargin: 200 // Pixels beyond viewport to keep particles
};
...
class ParticlePool {
    ...
    /**
     * Batch release multiple particles efficiently
     * More efficient than calling release() in a loop
     * @param {Array<Particle>} particles - Array of particles to release
     */
    releaseAll(particles) {
        if (!particles || particles.length === 0) return;
        
        // Reset all particles and add to pool in batch
        for (const particle of particles) {
            particle.reset();
            this.pool.push(particle);
        }
        
        // Remove particles from active array efficiently
        // Use filter instead of multiple splice calls
        const particleSet = new Set(particles);
        this.active = this.active.filter(p => !particleSet.has(p));
    }
...
class Particle {
    ...
    isDone() {
        // Enhanced culling: check lifespan and viewport bounds with configurable margin
        const margin = CONFIG.cullMargin || 200;
        const isOutOfBounds = this.x < -margin || 
                              this.x > window.innerWidth + margin || 
                              this.y < -margin || 
                              this.y > window.innerHeight + margin;
        return this.lifespan <= 0 || isOutOfBounds;
    }
...
class Firework {
    ...
    explode() {
        ...
        // Calculate particle count with multipliers
        let particleCount = Math.floor(baseParticles * this.intensity * tierMult * comboMult);
        
        // Enforce per-tier particle caps to prevent performance degradation
        const tierCap = CONFIG.particleCaps[this.tier] || CONFIG.particleCaps.medium;
        particleCount = Math.min(particleCount, tierCap);
        
        if (DEBUG && particleCount >= tierCap) {
            console.log(`[Fireworks] Particle count capped at ${tierCap} for ${this.tier} tier`);
        }
    ...
    update(deltaTime = 1.0) {
        ...
        // Batch collection of dead particles for efficient removal
        const deadParticles = [];
        ...
        for (let i = this.particles.length - 1; i >= 0; i--) {
            ...
            // Remove dead particles (batch collection for efficient pool release)
            if (p.isDone()) {
        ...
        // Batch release dead particles back to pool
        if (globalParticlePool && deadParticles.length > 0) {
            globalParticlePool.releaseAll(deadParticles);
        }
        
        // Batch collection for dead secondary particles
        const deadSecondaryParticles = [];
        ...
        for (let i = this.secondaryExplosions.length - 1; i >= 0; i--) {
            ...
            if (p.isDone()) {
                deadSecondaryParticles.push(p);
                this.secondaryExplosions.splice(i, 1);
            }
        }
        
        // Batch release dead secondary particles back to pool
        if (globalParticlePool && deadSecondaryParticles.length > 0) {
            globalParticlePool.releaseAll(deadSecondaryParticles);
        }
    }
...
class AudioManager {
    constructor() {
        ...
        // Audio playback limiting to prevent overwhelming processing
        this.activeSounds = new Map(); // Track currently playing sounds by name
        this.maxConcurrentSounds = 10; // Limit concurrent audio tracks
        this.lastPlayTime = new Map(); // Track last play time for throttling
        this.minPlayInterval = 50; // Minimum ms between same sound plays
    ...
    async play(name, volume = 1.0) {
        ...
        // Throttle: Check if this sound was played too recently
        const now = performance.now();
        const lastPlay = this.lastPlayTime.get(name) || 0;
        if (now - lastPlay < this.minPlayInterval) {
            if (DEBUG) console.log(`[Fireworks Audio] Throttled: ${name} (played ${Math.round(now - lastPlay)}ms ago)`);
            return;
        }
        ...
        if (this.activeSounds.size >= this.maxConcurrentSounds) {
            if (DEBUG) console.log(`[Fireworks Audio] Max concurrent sounds reached (${this.maxConcurrentSounds}), skipping: ${name}`);
            return;
        }
        ...
        try {
            ...
            // Track active sound
            const soundId = `${name}-${Date.now()}-${Math.random()}`;
            this.activeSounds.set(soundId, { source, gainNode, name });
    ...
    async playWithFadeOut(name, volume = 1.0, duration = 2.0, fadeOutDuration = 0.5) {
        ...
        const lastPlay = this.lastPlayTime.get(name) || 0;
        if (now - lastPlay < this.minPlayInterval) {
            if (DEBUG) console.log(`[Fireworks Audio] Throttled fade-out: ${name}`);
            return;
        }
        ...
        if (this.activeSounds.size >= this.maxConcurrentSounds) {
            if (DEBUG) console.log(`[Fireworks Audio] Max concurrent sounds reached, skipping fade-out: ${name}`);
            return;
        }
    ...
    /**
     * Get the current number of active sounds playing
     * @returns {number} Number of currently playing sounds
     */
    getActiveSoundCount() {
    ...
    /**
     * Configure audio playback limits
     * @param {number} maxConcurrent - Maximum number of concurrent sounds
     * @param {number} minInterval - Minimum interval between same sound plays (ms)
     */
    configurePlaybackLimits(maxConcurrent = 10, minInterval = 50) {
        this.maxConcurrentSounds = Math.max(1, Math.min(20, maxConcurrent));
        this.minPlayInterval = Math.max(10, Math.min(500, minInterval));
        if (DEBUG) console.log(`[Fireworks Audio] Playback limits: max=${this.maxConcurrentSounds}, interval=${this.minPlayInterval}ms`);
    }
