...
To prevent overwhelming processing resources during high gift load:
- **Maximum concurrent sounds**: 10 tracks (configurable)
- **Throttling interval**: 50ms minimum between same sound plays
- **Active sound tracking**: Real-time monitoring of playing audio
- **Automatic skipping**: Sounds are skipped when limits are reached

These limits ensure smooth audio playback even during extreme combo scenarios without audio stuttering or FPS drops.

### Volume Management
Consistent audio volume levels across all fireworks tiers:
- **Per-file volume multipliers**: Automatic balancing for inherently loud/quiet sounds
- **Tier-appropriate volumes**: Small/Medium/Big/Massive explosions properly scaled
- **Crackling fade-out**: Intelligent fade to match visual particle lifetime
