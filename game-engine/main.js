...
class GameEnginePlugin {
  ...
  createPendingChallenge(gameType, challengerUsername, challengerNickname, giftName, giftImageUrl, config) {
    ...
    // Set timeout to auto-accept against streamer
  ...
  registerRoutes() {
    ...
    // Serve wheel themes JavaScript file
    this.api.registerRoute('GET', '/overlay/game-engine/wheel-themes.js', (req, res) => {
      res.sendFile(path.join(__dirname, 'overlay', 'wheel-themes.js'));
    });
  ...
  registerSocketEvents() {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('wheel:spin-complete', async (data) => {
        const { spinId, segmentIndex } = data;
        await this.wheelGame.handleSpinComplete(spinId, segmentIndex);
      });
    });
  }
  ...
  handleGiftTrigger(data) {
    ...
    if (repeatEnd === false) {
      this.logger.debug(`Gift ${giftName} (ID: ${giftId}) is part of a streak, waiting for repeatEnd`);
      return;
    }
    
    // Pre-compute gift identifiers for efficient matching
    const giftIdStr = giftId !== undefined && giftId !== null ? String(giftId) : '';
    ...
    if (matchingWheel) {
      this.logger.debug(`Wheel gift trigger matched: ${giftName} (ID: ${giftId}) -> Wheel "${matchingWheel.name}" (ID: ${matchingWheel.id})`);
    ...
    const matchingTrigger = triggers.find(t => {
      ...
      const triggerValueLower = (t.trigger_value || '').toLowerCase();
      const triggerValueStr = String(t.trigger_value);
      
      // Match by gift ID (exact) or gift name (case-insensitive)
      return triggerValueStr === giftIdStr || 
             triggerValueLower === giftNameLower ||
             triggerValueStr === giftName;
    });
  ...
  /**
   * Helper method to check if a game for a viewer is already in the queue
   * @param {string} viewerUsername - Username to check
   * @param {string|null} gameType - Game type to check (optional). If null, checks for any game type for this user.
   *                                  If provided, only checks for that specific game type.
   * @returns {boolean} True if the viewer already has a game in the queue (of the specified type if provided)
   */
  ...
  handleGameStart(gameType, viewerUsername, viewerNickname, triggerType, triggerValue, giftPictureUrl = null) {
    this.logger.debug(`[QUEUE] handleGameStart called: ${viewerUsername} for ${gameType}, trigger: ${triggerType}=${triggerValue}`);
    
    // Check if player already has an active game or pending challenge
    const activeSession = this.db.getActiveSessionForPlayer(viewerUsername);
    if (activeSession) {
      this.logger.info(`[QUEUE] Player ${viewerUsername} already has an active game #${activeSession.id}`);
      return;
    }

    // Check if player already has a game in the queue to prevent duplicates
    if (this.queueContainsGame(viewerUsername, gameType)) {
      this.logger.info(`[QUEUE] Player ${viewerUsername} already has a ${gameType} game in queue, skipping duplicate`);
      return;
    }
    ...
    if (this.activeSessions.size > 0 || this.pendingChallenges.size > 0) {
      ...
      this.logger.info(`[QUEUE] Game queued: ${viewerUsername} for ${gameType} (Queue length: ${this.gameQueue.length}, Active: ${this.activeSessions.size}, Pending: ${this.pendingChallenges.size})`);
    ...
    // No active games - start immediately
    this.logger.info(`[QUEUE] No active games, starting ${gameType} for ${viewerUsername} immediately`);
    ...
    this.logger.info(`[QUEUE] Challenge created #${sessionId}: ${viewerUsername} challenges with ${triggerValue}`);
  }
  ...
  processNextQueuedGame() {
    this.logger.debug(`[QUEUE] processNextQueuedGame called (Queue: ${this.gameQueue.length}, Active: ${this.activeSessions.size}, Pending: ${this.pendingChallenges.size})`);
    
    if (this.gameQueue.length === 0) {
      this.logger.debug('[QUEUE] No games in queue');
      return;
    }

    // CRITICAL: Check if there's still an active game or challenge
    // This prevents processing queue items when conditions aren't met
    if (this.activeSessions.size > 0 || this.pendingChallenges.size > 0) {
      this.logger.warn(`[QUEUE] Cannot process queue: ${this.activeSessions.size} active sessions, ${this.pendingChallenges.size} pending challenges. Waiting...`);
      return;
    }
    ...
    if (!nextGame) {
      this.logger.debug('[QUEUE] Queue shift returned null, nothing to process');
      return;
    }

    this.logger.info(`[QUEUE] Processing queued game for ${nextGame.viewerUsername} (${this.gameQueue.length} remaining in queue)`);
    
    // Validate that the player doesn't already have an active game
    // (shouldn't happen, but safety check)
    const activeSession = this.db.getActiveSessionForPlayer(nextGame.viewerUsername);
    if (activeSession) {
      this.logger.warn(`[QUEUE] Skipping queued game: ${nextGame.viewerUsername} already has active game #${activeSession.id}`);
      return;
    }
    ...
    // Start the queued game directly (no re-queuing to prevent infinite loops)
    // Get configuration
    const config = this.db.getGameConfig(nextGame.gameType) || this.defaultConfigs[nextGame.gameType];

    // If challenge screen is disabled, start game directly
    if (!config.showChallengeScreen) {
      this.logger.debug(`[QUEUE] Starting game directly (challenge screen disabled)`);
      this.startGame(
        nextGame.gameType, 
    ...
    // Create a pending challenge
    this.logger.debug(`[QUEUE] Creating pending challenge`);
    const sessionId = this.createPendingChallenge(
      nextGame.gameType, 
      nextGame.viewerUsername, 
      nextGame.viewerNickname, 
    ...
    this.logger.info(`[QUEUE] Challenge created from queue #${sessionId}: ${nextGame.viewerUsername} with ${nextGame.triggerValue}`);
  }
  ...
  handleChatCommand(data) {
    const { uniqueId, comment, nickname, profilePictureUrl = '' } = data;
    ...
    if (matchingWheel) {
      ...
      this.handleWheelGiftTrigger(uniqueId, nickname, profilePictureUrl, `Command: ${cleanCommand}`, matchingWheel.id);
      return;
    }
    ...
    if (plinkoMatch) {
      this.handlePlinkoChatCommand(uniqueId, nickname, data, plinkoMatch[1]);
      return;
    }
    ...
    if (message.startsWith('/')) {
      ...
      if (c4Match) {
        ...
        this.handleViewerMove(uniqueId, nickname, 'connect4', column);
        return;
      }
      ...
      if (plinkoSlashMatch) {
        this.handlePlinkoChatCommand(uniqueId, nickname, data, plinkoSlashMatch[1]);
        return;
      }
    }
  ...
  startChessGame(viewerUsername, viewerNickname, triggerType, triggerValue, timeControl, config) {
    ...
    // Set up timer update interval (every 100ms for smooth internal updates)
    let updateCounter = 0;
    const timerInterval = setInterval(() => {
      ...
      // Emit timer update every 500ms (every 5th update) to avoid flooding
      if (updateCounter >= 5) {
        updateCounter = 0;
  ...
  endGame(sessionId, winner, reason, gameResult = null) {
    ...
    // Get XP rewards
    const xpRewards = this.db.getXPRewards(session.game_type);
    ...
    if (winner && gameResult) {
      ...
      if (session.game_type === 'chess') {
        ...
        winnerInfo = winner === 1 ? game.player1 : game.player2;
      }
    ...
    // Get win streak for the winner if viewer
  ...
  awardGameXP(session, winner, reason, xpRewards) {
    ...
    if (player1 !== 'streamer') {
      ...
      // Emit win streak notification if new record
  ...
  async handlePlinkoCommand(args, context) {
    try {
      ...
      if (args[0].toLowerCase() === 'max') {
        // Get user's current XP and bet max
        ...
        if (isNaN(betAmount)) {
          return {
            success: false,
            message: 'Invalid bet amount. Use /plinko <amount> or /plinko max',
            displayOverlay: true
          };
        }
      }
      ...
      return {
        success: true,
        message: `🎰 Plinko ball dropped! You bet ${betAmount} XP. Good luck!`,
        displayOverlay: true
      };
    } catch (error) {
